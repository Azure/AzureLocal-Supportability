...
In `C:\Monitoring\data\Configuration\MonAgentHost*.log` (and the MA event-table dumps) you will see this
...
Invoke-Command -ComputerName $node -ScriptBlock {
    ...
    $log = Get-ChildItem 'C:\Monitoring\data\Configuration\MonAgentHost*.log' -ErrorAction SilentlyContinue |
           Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if ($log) {
