...
### -Wait
Blocks until the background job completes, then returns the Job object (via Get-Job) without
receiving its output.
Ignored when -PassThru is also specified.
...
### -PassThru
Blocks until the background job completes, then returns the results the command produced.
...
### Without -AsJob, returns the command output. With -AsJob alone, returns the Job object
### immediately. -Wait blocks and returns the Job object without consuming its output.
### -PassThru blocks and returns the received job output.
